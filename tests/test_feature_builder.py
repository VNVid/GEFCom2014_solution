...
ROUND2_FEATURE_CONFIG = Path("configs/features/round2_candidates.yaml")
...
def test_round2_candidate_config_builds_exactly_70_complete_features() -> None:
    with ROUND2_FEATURE_CONFIG.open("r", encoding="utf-8") as stream:
        groups = yaml.safe_load(stream)["feature_groups"]
    ...
    assert features.loc[2, "holiday_name"] == "New Year's Day"
    assert features.loc[2, "is_holiday"] == 1
    assert features.loc[2, "load_mean_90d"] == pytest.approx(200.0)
    assert features.loc[2, "load_seasonal_daytype_8d_q25"] <= features.loc[
        2, "load_seasonal_daytype_8d_q75"
    ]
    assert features.loc[2, "load_annual_anchor_mean"] == pytest.approx(150.0)
    assert features.loc[2, "load_annual_anchor_std"] == pytest.approx(50.0)
    assert features.loc[2, "load_seasonal_level_delta_28d"] == pytest.approx(100.0)
    assert features.loc[2, "temperature_clim_15d_q10"] == pytest.approx(63.0)
    assert features.loc[2, "temperature_clim_15d_q90"] == pytest.approx(63.0)
    assert features.loc[2, "temperature_clim_15d_hdd65"] == pytest.approx(2.0)
    assert features.loc[2, "temperature_clim_15d_cdd65"] == pytest.approx(0.0)
    assert features.loc[
        2, "load_last_day_same_hour_decay_2d"
    ] == pytest.approx(features.loc[2, "load_last_day_same_hour"])
    assert categorical_feature_names(groups) == (
        *TARGET_CATEGORICAL_FEATURES,
        *HOLIDAY_CATEGORICAL_FEATURES,
    )
